    ...
    <div className="bg-slate-800/50 border border-slate-600/30 rounded-lg p-4 mb-4 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="flex justify-between items-center">
          <h4 className="text-sm font-medium text-slate-200">Track Disruption</h4>
          <button
            type="button"
            onClick={onClose}
        ...
        <select
          value={category}
          onChange={(e) => setCategory(e.target.value as Disruption["category"])}
...
interface ManualEntryFormProps {
  isVisible: boolean;
  onClose: () => void;
  onSubmit: (title: string, category: Disruption["category"], startTime: Date, duration: number) => void;
}
...
const TasksSidebar: React.FC<TasksSidebarProps> = ({ viewMode = "triage" }) => {
  ...
  const handleStartDisruption = async (title: string, category: Disruption["category"]) => {
  ...
  const handleManualEntry = async (
    ...
    await createManualDisruption(title, category, startTime, duration, currentSession?.taskId);
  };
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "bug-fix": return <AlertTriangle className="h-4 w-4" />;
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case "slack": return "text-purple-400";
      case "bug-fix": return "text-red-400";
      ...
      <div className="bg-slate-900/50 border border-slate-700/50 backdrop-blur-sm rounded-lg p-4 h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 border-b border-slate-700/50 pb-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Track disruptions & stay focused</p>
          </div>
        </div>
          ...
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-red-400 text-sm">Active Disruption</span>
              </div>
              <span className="text-xs text-slate-300 font-mono">
                {activeDisruptionTimer}
              </span>
            </div>
            <p className="text-slate-200 text-sm mb-3">{activeDisruption.title}</p>
            <button
        ...
        <div className="space-y-3 mb-4">
          ...
          {!activeDisruption && (
            ...
            >
              ...
              <span className="text-blue-400 font-medium text-sm">Track Disruption</span>
            </button>
          )}
          ...
          >
            ...
            <span className="text-green-400 font-medium text-sm">Manual Entry</span>
          </button>
        </div>
        ...
        {disruptionSummary && (
          <div className="space-y-3 mb-4">
            <h4 className="text-sm font-medium text-slate-300">Today's Summary</h4>
            ...
            {disruptionSummary.averageDisruptionLength > 0 && (
              <div className="bg-slate-700/30 rounded-lg p-3">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <span className={`text-xs font-medium capitalize ${getCategoryColor(disruptionSummary.mostCommonCategory)}`}>
        ...
        {todaysDisruptions.length > 0 && (
          <div className="space-y-3 mb-4">
            <h4 className="text-sm font-medium text-slate-300">Recent Disruptions</h4>
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {todaysDisruptions
                ...
                .map((disruption) => (
                  <div key={disruption.id} className="bg-slate-700/20 rounded-lg p-2">
                    ...
                    <div className="text-xs text-slate-500">
                      {disruption.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
                ))}
            </div>
        ...
        <div className="mt-auto bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-lg p-3">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            {disruptionSummary?.totalDisruptions && disruptionSummary.totalDisruptions > 3
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-700/50 backdrop-blur-sm rounded-lg p-4 h-full flex flex-col">
      ...
      {/* Recent Activity */}
      <div className="mb-6">
        <h4 className="text-slate-300 text-sm font-medium mb-3">Recent Activity</h4>
        <div className="space-y-2">
          <div className="flex items-center p-2 bg-slate-800/20 rounded">
            <div className="w-2 h-2 bg-green-400 rounded-full mr-3"></div>
            <span className="text-slate-400 text-xs">Completed API integration</span>
          </div>
          <div className="flex items-center p-2 bg-slate-800/20 rounded">
          ...
          <div className="flex items-center p-2 bg-slate-800/20 rounded">
            ...
            <span className="text-slate-400 text-xs">Scheduled team meeting</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 bg-gradient-to-r from-green-500/10 to-cyan-500/10 border border-green-500/30 rounded-lg p-3">
        <div className="flex items-center mb-2">
          ...
          <span className="text-green-400 text-sm font-medium">Signal Focus</span>
        </div>
