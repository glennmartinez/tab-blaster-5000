...
import { FavoritesService, FavoriteTab, Tag } from '../services/FavoritesService';
...
export const useFavorites = () => {
  ...
  const isFavorite = useCallback(async (url: string): Promise<boolean> => {
  ...
  // Remove a favorite
  const removeFavorite = useCallback(async (favoriteId: string): Promise<void> => {
    await favoritesService.removeFavorite(favoriteId);
    await loadData(); // Refresh data
  }, [favoritesService, loadData]);

  // Remove favorite by URL
  const removeFavoriteByUrl = useCallback(async (url: string): Promise<void> => {
  ...
  const updateFavoriteTags = useCallback(async (favoriteId: string, tags: string[]): Promise<void> => {
  ...
  const getFavoritesByTags = useCallback(async (tags: string[]): Promise<FavoriteTab[]> => {
    return await favoritesService.getFavoritesByTags(tags);
  }, [favoritesService]);

  // Get favorite state for a specific URL
  const getFavoriteState = useCallback((url: string) => {
