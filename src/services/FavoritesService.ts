...
export class FavoritesService {
  ...
  async addFavorite(tab: { id?: number; title: string; url: string; favicon?: string }, tags: string[] = []): Promise<FavoriteTab> {
    ...
    if (existingIndex !== -1) {
      ...
      favorites[existingIndex].tags = [...new Set([...favorites[existingIndex].tags, ...tags])];
  ...
  async getFavoritesByTags(tags: string[]): Promise<FavoriteTab[]> {
    ...
    return favorites.filter(favorite => 
      tags.some(tag => 
        favorite.tags.some(favTag => 
          favTag.toLowerCase() === tag.toLowerCase()
        )
      )
    );
  }
